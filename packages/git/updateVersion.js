...
const updataDependenciesVersion = require('./updataDependenciesVersion.js')
        ...
        async (err, data) => {
          ...
          if (matchAry && matchAry.length > 0) {
            for (let matcher of matchAry) {
              ...
              versionObj[package].gitUrl = gitCommitUrl
        ...
        await new Promise((resolve, reject) => {
          exec(
            ...
            async function (err, data) {
              ...
              // )
              // await gitTask(`git commit -m "${package}_${data}"`)
              // let commitId = await getCommitInfo()
              versionChangeInfo.push({
                path: `./${versionObj[package].packagePath}`,
                package,
                // commitId,
                curVersion: curPackageJson.curVersion,
                packageName: packageJsons.find(
                  (item) => item.folderName === package
                ).name,
              })
              resolve()
...
module.exports = async function updateVersion() {
  ...
  result = await updataDependenciesVersion(result)
  ...
  await gitTag(result.versionChangeInfo)
